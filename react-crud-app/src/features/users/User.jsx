...
const User = ({ user, editUser, setIsPositive, setMessage, setShowMessage, reload, reloadNow }) => {
 ...
 return (
  <>
   <tr>
   <td><img src="https://images.unsplash.com/photo-1635244621620-ccadff2eb29d?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=580&q=80" className='img-fluid vh-7 my-2 rounded-pill' alt="" /></td>
    <td>{user.username}</td>
    ...
    <td>
     <Button clickHandler={() => editUser(user)} startIcon={<PenIcon />} dataCy="btnEdit" buttonId={`btnEdit${user.username}`}/>
    </td>
    <td>
     <Button clickHandler={() => deleteUser(user)} startIcon={<TrashIcon />} dataCy="btnDelete" buttonId={`btnDelete${user.username}`} />
    </td>
   </tr>
  </>
 );
