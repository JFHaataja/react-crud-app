...
const [newProductSupplierId, setNewProductSupplierId] = useState(muokattavaProduct.supplierId)
const [newProductCategoryId, setNewProductCategoryId] = useState(muokattavaProduct.categoryId)
const [newQuantityPerUnit, setNewQuantityPerUnit] = useState(muokattavaProduct.quantityPerUnit)
const [newProductUnitPrice, setNewProductUnitPrice] = useState(muokattavaProduct.unitPrice)
const [newProductUnitsInStock, setNewProductUnitsInStock] = useState(muokattavaProduct.unitsInStock)
const [newProductUnitsOnOrder, setNewProductUnitsOnOrder] = useState(muokattavaProduct.unitsOnOrder)
const [newProductReorderLevel, setNewProductReorderLevel] = useState(muokattavaProduct.reorderLevel)
const [newProductDiscontinued, setNewProductDiscontinued] = useState(muokattavaProduct.discontinued)
const [radioButtonValue, setRadioButtonValue] = useState(muokattavaProduct.discontinued ? 1 : 0)
    ...
    <Form onSubmit={handleSubmit} className="text-white text-start m-auto mt-3">
     <Row>
       <Col className="mx-0 mx-lg-5 px-0 px-lg-5">
         {/* <FormGroup formLabelText={"Product ID"} inputType="number" inputValue={newProductId} onChangeHandler={({ target }) => setNewProductId(target.value)} disabled /> */}
         ...
         <FormGroup formLabelText={"Reorder Level"} inputType="text" inputValue={newProductReorderLevel} inputPlaceholder="Reorder Level" onChangeHandler={({ target }) => setNewProductReorderLevel(target.value)} />
         <div>
            <p className="mb-2">Discontinued</p>
        </div>
         <div onChange={({ target }) => setNewProductDiscontinued(parseInt(target.value))}>
            <div className='mb-2'>
                <input className='form-check-input me-2' id="radio1" type="radio" name={newProductDiscontinued.toString()} defaultValue={1} defaultChecked={radioButtonValue === 1} onChange={() => setRadioButtonValue(1)}/>
                <label className="form-check-label" htmlFor="radio1">True</label>
            </div>
            <div>
              <input className='form-check-input me-2' type="radio" id="radio2" name={newProductDiscontinued.toString()} defaultValue={0} defaultChecked={radioButtonValue === 0} onChange={() => setRadioButtonValue(0)}/>
